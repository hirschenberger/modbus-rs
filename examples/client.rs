...
fn main() {
    let matches = App::new("client")
                    ...
                    .args_from_usage(
                        ...
                        --read-discrete-inputs=[ADDR] [QUANTITY] 'Read QUANTITY inputs from ADDR'
                        --write-single-coil=[ADDR] [On,Off] 'Write the coil value (On or Off) to \
                          ADDR'
                        --write-multiple-coils=[ADDR] [On,Off..] 'Write multiple coil values \
                          (On or Off) to ADDR (use \"..\" without spaces to group them like \
                          \"On,Off,On,Off\")'
                        --read-input-registers=[ADDR], [QUANTITY] 'Read QUANTITY input registers\
                          from ADDR'
                        ...
                        --write-multiple-registers=[ADDR] [V1,V2...] 'Write multiple register \
                          values to ADDR (use \"..\" without spaces to group them like \
                          \"23, 24, 25\")'")
                    .get_matches();
    ...
    if let Some(args) = matches.values_of("read-coils") {
        ...
        let addr: u16 = args[0].parse().expect(matches.usage());
        let values: Vec<Coil> = args[1].split(',').map(|s| s.parse().expect(matches.usage())).collect();
        client.write_multiple_coils(addr, &values).expect("IO Error");
    }
    else if let Some(args) = matches.values_of("read-holding-registers") {
        let addr: u16 = args[0].parse().expect(matches.usage());
        let qtty: u16 = args[1].parse().expect(matches.usage());
        println!("{:?}", client.read_holding_registers(addr, qtty).expect("IO Error"));
    }
        ...
        let addr: u16 = args[0].parse().expect(matches.usage());
        let values: Vec<u16> = args[1].split(',').map(|s| s.parse().expect(matches.usage())).collect();
